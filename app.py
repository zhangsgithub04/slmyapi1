...
def get_presidents():
    r = requests.get(f"{API_BASE}/presidents", timeout=20)
    r.raise_for_status()
    return r.json()
...
def delete_president(president_id):
    r = requests.delete(f"{API_BASE}/presidents/{president_id}", timeout=20)
    return r
...
with st.form("president_form"):
    firstname = st.text_input("First Name", value=edit_row["firstname"] if edit_row else "")
    lastname = st.text_input("Last Name", value=edit_row["lastname"] if edit_row else "")
...
if save:
    try:
        if st.session_state.edit_id is None:
            r = create_president(firstname.strip(), lastname.strip(), birthdate.strip())
        else:
            r = update_president(st.session_state.edit_id, firstname.strip(), lastname.strip(), birthdate.strip())
